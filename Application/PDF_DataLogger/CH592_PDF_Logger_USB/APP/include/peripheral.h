...
typedef struct
{
    ...
    uint8_t DISK_BASE_BUF[DISK_BASE_BUF_LEN]; /* The disk data buffer of external RAM, the buffer length is the length of a sector */
    uint8_t DISK_FAT_BUF[ DISK_BASE_BUF_LEN ]; /* The disk FAT data buffer of external RAM, the buffer length is the length of a sector */
}PDF_BUFFER_t;

...
void USB_DevTransProcess(void)
{
    ...
    if(intflag & RB_UIF_TRANSFER)
    {
                ...
                case UIS_TOKEN_IN | DEF_UEP0:
                {
                    switch(USBFS_SetupReqCode)
                    {
                        case USB_GET_DESCRIPTOR:
                            len = USBFS_SetupReqLen >= DEF_USBD_UEP0_SIZE ? DEF_USBD_UEP0_SIZE : USBFS_SetupReqLen; // The transmission length
                            pdf_memcpy(pEP0_DataBuf, pUSBFS_Descr, len);                          /* Load upload data */
        ...
        if(R8_USB_INT_ST & RB_UIS_SETUP_ACT) // Setup package processing
        {
                ...
                switch(USBFS_SetupReqCode)
                {
                    case USB_GET_DESCRIPTOR:
                    {
                        ...
                        if(USBFS_SetupReqLen > len)
                            USBFS_SetupReqLen = len; // The total length needs to be uploaded
                    ...
                    {
                        if((USBFS_SetupReqType & USB_REQ_RECIP_MASK) == USB_REQ_RECIP_ENDP) // Endpoint
                        {
            ...
            {
                if(USBFS_SetupReqType & 0x80) // Upload
                {
                    len = (USBFS_SetupReqLen > DEF_USBD_UEP0_SIZE) ? DEF_USBD_UEP0_SIZE : USBFS_SetupReqLen;
                    USBFS_SetupReqLen -= len;
                }

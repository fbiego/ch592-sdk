...
uint32_t self_prov_iv_index = 0x00000000; // iv_index0
uint16_t self_prov_addr = 0;         // 
uint8_t  self_prov_flags = 0x00;          // key
...
static int vendor_model_srv_send(uint16_t addr, uint8_t *pData, uint16_t len)
{
    struct send_param param = {
        ...
        .addr = addr,          // 
        .trans_cnt = 0x05,                // 
        .period = K_MSEC(500),            // (200+50*TTL)ms
        .rand = (0),                      // 
...
void App_trans_model_reveived(uint8_t *pValue, uint16_t len, uint16_t addr )
{
    ...
    switch(app_mesh_manage.data.buf[0])
    {
        ...
        case CMD_ASK_STATUS:
        {
            ...
            int status;
            app_mesh_manage.ask_status_ack.cmd = CMD_ASK_STATUS_ACK;
            app_mesh_manage.ask_status_ack.status = STATUS_SUCCESS; //
        ...
        case CMD_SET_SUB:
        {
            ...
            app_mesh_manage.set_sub_ack.cmd = CMD_SET_SUB_ACK;
            app_mesh_manage.set_sub_ack.status = status; //

...
* Version: V1.0
* Date: 2018/11/12
* Description: Main machine multi-connection routine, actively scans surrounding devices, connects to the given three slave device addresses,
* Find custom services and features, execute read and write commands, and use them in conjunction with slave routines.
And modify the slave device address to the target address of the routine, and the default address of the three slave device is
...
uint16_t Central_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    // Task processing for connection 1
...
static void centralGATTDiscoveryEvent(uint8_t connItem, gattMsgEvent_t *pMsg)
{
    ...
    // Connect 1 enumeration
    else if(connItem == CONNECT1_ITEM)
    {
    }
    // Enumeration of 2 connections
    else if(connItem == CONNECT2_ITEM)
    {
    }
}
